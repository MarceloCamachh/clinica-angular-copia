  ...
  implements OnInit, BaseDialogData<Appointment>
{
  ...
  appointmentForm = this.formBuilder.group({
    status: this.formBuilder.control<AppointmentStatus>(
  ...
  onSubmit() {
    if (this.appointmentForm.valid) {
      ...
      const appointmentData: AppointmentToAddOrUpdate = {
        ...formData,
        ...
        status: formData.status ?? AppointmentStatus.SCHEDULED,
        medicines: formData.medicines as Medicine[] || []
      };

      this.appointmentService
        .updateAppointment(appointmentData, this.data?.id ?? '')
        .subscribe({
          next: (appointment) => {
            if (!appointment) {
